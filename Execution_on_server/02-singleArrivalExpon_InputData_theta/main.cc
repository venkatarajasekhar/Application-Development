...
void outputDataFormat(){
    ...
    file.open("Data_singleArrivalExpon_theta.txt" , ios ::out | ios::trunc) ;
    file<<" "<<endl;
    file.close() ;
}


void outputDataMessage( double m , double totalN , int CCR_Initial_msg, int CCR_Terminate_msg , double E_N , double E_Cu ,double E_Crc ,  int randomCheckNum  , int BeConsumed_Num , int Recharge_Num){
    
    file.open("Data_singleArrivalExpon_theta.txt" , ios ::out | ios::app) ;
    file<<m<<" "<<E_N<<" "<<E_Cu<<" "<<E_Crc<<" "<<totalN<<" "<<randomCheckNum<<" "<<BeConsumed_Num<<" "<<Recharge_Num<<" "<<CCR_Initial_msg<<" "<<CCR_Terminate_msg<<endl;
    file.close() ;
}
...
int main(int argc, char**argv) {
    ...
    double m[x_axis] ; //Theta = m events' credit
    int x=0 ;
    file.open( "InputData_theta.txt" , ios ::in) ;
        while ( !file.eof() ) {
                file >> m [x++] ;
            }
    file.close() ;
    ...
    for (int i = 0 ; i < x_axis ; i++ ) {
        ...
        double Theta = m[i] * eventCreditUnit ;
...
//cout << "\nRandom check :\n" << *ptr << "\n";
        ...
        outputDataMessage(m[i], totalN , CCR_Initial_msg, CCR_Terminate_msg , E_N , E_Cu , E_Crc , randomCheckNum  , BeConsumed_Num , Recharge_Num ) ;
    }//for x-axis
