...
case 0
     for ii=1:m-2
      prob = prob + 1/(m-ii+1)       *exp(-lambda*T)*( lambda * T )^(m-ii-1)/factx(m-ii-1)  ;
     end
       prob = prob + 1/2   *exp(-lambda*T)*(lambda*T)^(0)/factx(0) +1* exp(-lambda*T);

case m-1
   for ii=1:m-2
      prob = prob + 1/(m-ii+1)     *exp(-lambda*T)*( lambda * T )^(m-ii-1)/factx(m-ii-1) ;
   end
      prob = prob + 1/2    *exp(-lambda*T)*(lambda*T)^(0)/factx(0)  ;
