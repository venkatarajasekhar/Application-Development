function prob = P_113(j, m,lambda ,T1,T2)
...
otherwise
    for ii=1:j
      prob = prob + (tmStar(m,lambda,T1)/(1/lambda+tmStar(m,lambda,T1))) * 1/(m-ii)  /SumNormalize(0,m-1,lambda,tmStar(m,lambda,T1))    *exp(-lambda*tmStar(m,lambda,T1) ) *  ( lambda * tmStar(m,lambda,T1))^(m-ii-1)/factx(m-ii-1)  ;
   end
...
% case 0
%    for ii=1:m-1
%       prob = prob + 1/(m-ii+1) /SumNormalize(0,m-1,lambda,tmStar(m,lambda,T1))  *exp(-lambda*T2)*( lambda * T2 )^(m-ii-1)/factx(m-ii) ;
%    end
%       prob = prob +   1/SumNormalize(0,m-1,lambda,tmStar(m,lambda,T1)) *  exp( -lambda*T2 )  ;
% 
%     
